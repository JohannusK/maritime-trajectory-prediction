...
MAX_GAP_MINUTES = 10  # Maximum gap in minutes to consider same trajectory
...
DEFAULT_LOG_FILE = Path("/home/johannus/PycharmProjects/aisData/combined_aiscatcher.log")

# Setup logging
def setup_logging(level=logging.INFO, log_file: Path | str | None = None):
    """Configure logging for the AIS processing pipeline.

    A stream handler is always attached; a file handler is added when
    ``log_file`` is provided. If ``log_file`` is omitted, logs default to
    ``<repo>/logs/ais_pipeline.log``.
    """
    ...
    if log_file is None:
        log_file = project_root / "logs" / "ais_pipeline.log"
    ...
    # Stash the resolved log path for callers who want to surface it.
...
def expected_output_paths(output_prefix: str | Path) -> dict[str, Path]:
    """Return paths for artifacts generated by the ingestion pipeline."""
    prefix = Path(output_prefix)
    prefix_str = str(prefix)
    return {
        "parquet": Path(f"{prefix_str}_all.parquet"),
        "csv": Path(f"{prefix_str}_all.csv"),
        "jsonl": Path(f"{prefix_str}_vessels.jsonl"),
        "static_csv": Path(f"{prefix_str}_static.csv"),
        "visual": Path(f"{prefix_str}_trajectories.png"),
    }


def get_artifact_status(output_prefix: str | Path) -> dict[str, list[Path]]:
    """Summarise cached artifacts for the provided output prefix."""
    paths = expected_output_paths(output_prefix)

    required_keys = ("csv", "jsonl", "static_csv", "visual")
    missing_required = [paths[key] for key in required_keys if not paths[key].exists()]

    data_files: list[Path] = []
    for key in ("csv", "jsonl", "static_csv", "parquet"):
        path = paths[key]
        if path.exists():
            data_files.append(path)

    visuals = [paths["visual"]] if paths["visual"].exists() else []
...
def export_to_formats(trajectories, output_prefix):
    ...
    paths = expected_output_paths(prefix_path)

    output_files: list[str] = []

    output_dir = prefix_path.parent
    if output_dir and str(output_dir) not in (".", ""):
    ...
    try:
        ...
        logging.getLogger("ais_processor").warning(
            "Skipping Parquet export; install pyarrow or fastparquet to enable it (%s)",
            exc,
        )
    else:
        output_files.append(str(parquet_path))
    ...
    static_csv_path = paths['static_csv']
    static_df.to_csv(static_csv_path, index=False)
    output_files.append(str(static_csv_path))
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Path to AIS-catcher log file (default: {DEFAULT_LOG_FILE})",
    )
    ...
    parser.add_argument("--log-output", dest="log_output", type=Path, help="Path to write detailed logs")
    parser.add_argument("--force", action="store_true", help="Recompute outputs even if cached artifacts exist")
    parser.add_argument("--no-visualize", dest="visualize", action="store_false", help="Skip visualization export")
    ...
    logger = setup_logging(level=log_level, log_file=args.log_output)
    log_destination = getattr(logger, "log_path", None)
    if log_destination is not None:
        logger.info(f"Writing detailed logs to {log_destination}")
    
    output_prefix = Path(args.output)
    artifact_status = get_artifact_status(output_prefix)
    if not args.force and not artifact_status["missing_required"]:
        logger.info("Cached ingestion artifacts detected for prefix '%s'; skipping processing.", output_prefix)
        for path in artifact_status["data_files"]:
            logger.info("  existing data file: %s", path)
        for path in artifact_status["visuals"]:
            logger.info("  existing visualization: %s", path)
        return
    ...
    for i, traj in enumerate(tqdm(trajectories, desc="Processing trajectories")):
        ...
        if len(filtered_traj) < len(traj_with_features) * 0.9:  # More than 10% filtered
            logger.debug(
                "Trajectory %s: %s outliers filtered out (%s/%s points remain)",
                i,
                len(traj_with_features) - len(filtered_traj),
                len(filtered_traj),
                len(traj_with_features),
            )
    ...
    output_files = export_to_formats(final_trajectories, output_prefix)
    ...
    if args.visualize:
        if not final_trajectories:
            logger.warning("No trajectories to visualize; skipping plot export")
        else:
            logger.info("\nVisualizing trajectories...")
            fig, _ = plot_trajectories(
                final_trajectories,
                output_file=str(paths["visual"]),
                max_trajectories=args.max_trajectories
            )

            logger.info(f"Trajectory visualization saved to {paths['visual']}")
